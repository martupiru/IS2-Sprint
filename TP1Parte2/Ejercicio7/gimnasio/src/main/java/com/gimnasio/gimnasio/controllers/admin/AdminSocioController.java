...
public class AdminSocioController {
    ...
    public String guardarSocios(HttpSession session, @ModelAttribute("socio") Socio socio,
                                @RequestParam("idSucursal") String idSucursal,
                                @RequestParam("idLocalidad") String idLocalidad,
        ...
        try {
            // Crear dirección y socio usando servicios (esto ya lo hacías bien)
            Direccion direccionCreada = direccionService.crearDireccionObj(
                    socio.getDireccion().getCalle(),
                    socio.getDireccion().getNumeracion(),
                    socio.getDireccion().getBarrio(),
                    socio.getDireccion().getManzanaPiso(),
                    socio.getDireccion().getCasaDepartamento(),
                    socio.getDireccion().getReferencia(),
                    idLocalidad
            );
            ...
            Socio socioCreado = socioService.crearSocioObj(
                    socio.getNombre(), socio.getApellido(), socio.getFechaNacimiento(),
                    socio.getTipoDocumento(), socio.getNumeroDocumento(), socio.getTelefono(),
                    socio.getCorreoElectronico(), direccionCreada, sucursal
            );

            // Crear usuario con el nuevo sistema de BCrypt
    ...
    public String actualizarSocio(HttpSession session,
        ...
        try {
            ...
            // Actualizar socio
            socioService.modificarSocio(
                    socio.getNombre(),
                    socio.getApellido(),
                    socio.getFechaNacimiento(),
                    socio.getTipoDocumento(),
                    socio.getNumeroDocumento(),
                    socio.getTelefono(),
                    socio.getCorreoElectronico(),
                    socioExistente.getNumeroSocio(),
                    direccionExistente,
                    sucursal
            );
            ...
            usuarioRepository.findByPersona(socioExistente).ifPresent(usuario -> {
                usuario.setNombreUsuario(socio.getCorreoElectronico());
                try {
                    if (clave != null && !clave.isEmpty()) {
                        // Aquí se llama al servicio (que lanza Exception), así que lo metemos dentro de un try/catch local
                        usuarioService.modificarClave(usuario.getId(), clave);
                    }
                    usuarioRepository.save(usuario);
                } catch (Exception e) {
                    // Este catch maneja específicamente el caso de error al actualizar clave
                    redirectAttrs.addFlashAttribute("error", "Error al actualizar la clave del usuario: " + e.getMessage());
                }
            });
